...
export function deleteInvitation(invitationId: string) {
  saveInvitations(getStoredInvitations().filter(invitation => invitation.id !== invitationId));
}
...
export function setInvitationStatus(invitationId: string, status: InvitationEditorialStatus) {
  const invitation = getInvitationById(invitationId);
...
export function getUpcomingPublishedInvitationCountForCurrentUser() {
  ...
  return invitations.filter(invitation => {
    return (
      invitation.status === 'published' && getInvitationTimingStatus(invitation) === 'upcoming'
    );
  }).length;
}
...
export function getPublishedInvitationCountBySpecialty(specialtyId: SpecialtyId) {
  return getStoredInvitations().filter(invitation => {
    return invitation.status === 'published' && invitation.targetSpecialtyIds.includes(specialtyId);
  }).length;
}
...
function normalizeInvitation(rawInvitation: unknown): Invitation {
  ...
  const rawScheduledDate =
    ...
    '2026-08-05';
  const rawStartTime = normalizeTimeValue(
    asString(invitation.startTime) ?? asString(invitation.time) ?? '09:00'
  );
...
function normalizeEditorialStatus(
  value: string | null,
  scheduledDate: string
): InvitationEditorialStatus {
