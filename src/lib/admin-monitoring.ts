...
import type { AdminDashboardMetrics, MonitoringStatus, PatientMonitoringSummary } from '@/types/admin'
...
export function getAdminDashboardMetrics(): AdminDashboardMetrics {
  ...
  return {
    ...
    patientsReportedToday: summaries.filter((summary) => summary.lastReportAt && toDateKey(new Date(summary.lastReportAt)) === todayKey).length,
    patientsMissingRecentReport: summaries.filter((summary) => summary.monitoringStatus === 'stale' || summary.monitoringStatus === 'no-data').length,
    patientsNeedingAttention: summaries.filter((summary) => summary.monitoringStatus === 'attention').length,
...
export function getPatientMonitoringDetail(patientId: string) {
  ...
  const symptomHistory = getSymptomRecords()
    .filter((record) => record.patientId === patientId)
    .sort(compareDatesDesc)
  const summary = getPatientMonitoringSummaries().find((entry) => entry.patient.id === patientId) ?? null
...
function getMonitoringStatus(lastReportAt: string | null, latestSymptomRecord: SymptomRecord | null): MonitoringStatus {
  if (!lastReportAt) {
    return 'no-data'
  }
